            ...
            <div id="large-header" class="loBox" >
                <div class="logo_box">
                    ...
                    <form @submit.prevent="submit" autocomplete="off" method="post ">
                        <p style="color:#555;">邮箱:{{user.email}}</p>
...
export default {
  ...
  mounted(){
	...
	this.user.token = this.getUrl('token')
  ...
  methods: {
        submit() {
            this.$axios.post("/api/users/resetPassword",this.user)
            .then(res=>{
                if(res.data.code==1000){
					...
					user.email=''
					user.password=''
					this.$router.push({'path':'/login?password=reset&&email='+this.user.email})
