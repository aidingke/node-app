...
export default {
    ...
    methods: {
        ...
        async goPage(x){
            //存储页码
            sessionStorage.setItem("vuePage", x)
    ...
    mounted() {
        ...
        if(sessionStorage.getItem("vuePage")){
            this.artPage = Number(sessionStorage.getItem("vuePage"))
            this.goPage(sessionStorage.getItem("vuePage"))
        }else{
            this.goPage(1)
        }
        $('footer').show()
